...
class checkRequestInIfStatement extends ASGrammarListener {
    ...
    checkIfStatement(ctx, tellCallback, start) {
        ...
        for (let statementCtx of ctx.statement()) {
            ...
            if (ifBlockCtx) {
                const count = this.checkIfBlock(ifBlockCtx, tellCallback);
                requests += count[0];
                keystrokes += count[1];
            }
    ...
    checkElseRequests(ctx, tellCallback, props, start) {
        ...
        const elseProps = this.checkIfStatement(ctx, tellCallback, start);
    ...
    checkElse(ctx, tellCallback, props, start) {
        if (ctx.constructor.name === 'ElseStatementContext') {
            ...
            props = this.checkElseRequests(elseStatementCtx.statementList(), tellCallback, props, start);
        }
        else {
            const elseIfCtx = ctx;
            props = this.checkElseRequests(elseIfCtx.ifStatement().statementList(), tellCallback, props, start);
        }
        return props;
    }
    checkIfBlock(ifBlockCtx, tellCallback, start, ctx) {
        ...
        if ((ctx === null || ctx === void 0 ? void 0 : ctx.constructor.name) === 'IfStatementContext' || !ctx) {
            ...
            count = this.checkIfStatement(ifBlockCtx.ifStatement().statementList(), tellCallback, start);
            ...
            if (elseIfArray) {
                elseIfArray.forEach((elseIfCtx) => {
                    count = this.checkElse(elseIfCtx, tellCallback, count, start);
                });
            }
            // else
            const elseStatementCtx = ifBlockCtx.elseStatement();
            if (elseStatementCtx) {
                count = this.checkElse(elseStatementCtx, tellCallback, count, start);
            }
        }
        // For others, check if the ifBlock has already been processed (if request in ifStatement/preceeding elseIfStatment)
        else {
            if (this.checkIfStatement(ifBlockCtx.ifStatement().statementList(), tellCallback, start).every((element) => !element)) {
                let afterCtx = false;
                if (elseIfArray) {
                    for (let elseIfCtx of elseIfArray) {
                        if (elseIfCtx.start.start === ctx.start.start &&
                            ...
                            count = this.checkElse(elseIfCtx, tellCallback, count);
                ...
                if (!calced) {
                    const elseStatementCtx = ctx;
                    count = this.checkElse(elseStatementCtx, tellCallback, count);
                }
            }
            else {
                calced = true;
    ...
    checkErrorHandler(parentCtx, tellCallback, ctx, start) {
        ...
        if ((tryStatementListCtx.start.start === (ctx === null || ctx === void 0 ? void 0 : ctx.start.start) &&
            ...
            !ctx) {
            let [requests, keystrokes] = this.checkIfStatement(tryStatementListCtx, tellCallback, start);
            const errorStatementListCtx = parentCtx.statementList(1);
            if (errorStatementListCtx) {
                const count = this.checkIfStatement(tryStatementListCtx, tellCallback, start);
                if (count[0] > requests) {
                    requests = count[0];
                }
