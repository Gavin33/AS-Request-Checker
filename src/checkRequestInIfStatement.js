...
const ASGrammarListener = require('./ASGrammarListener')['default']
...
class checkRequestInIfStatement extends ASGrammarListener {
    ...
    checkStart(ctx, areRequests, count, start, first) {
    ...
    checkIfStatement(ctx, tellCallback, start, first) {
        var _a, _b, _c;
        let requests = 0;
        let keystrokes = 0;
        for (let statementCtx of ctx.statement()) {
            ...
            if (ifBlockCtx) {
                const count = this.checkIfBlock(ifBlockCtx, tellCallback.bind(this), false, false, first);
                if (count) {
            ...
            // handle statements. Specifically errorHandlers, tells
                ...
                const count = this.checkErrorHandler(errorHandlerCtx, tellCallback.bind(this), false, first);
                if (count) {
                ...
                const count = this.checkStart(statementCtx, !!(requests || keystrokes), [requests, keystrokes], start, first);
    ...
    checkElseRequests(ctx, tellCallback, props, start, first) {
        ...
        const elseProps = this.checkIfStatement(ctx, tellCallback.bind(this), start, first);
        if (elseProps) {
    ...
    checkElse(ctx, tellCallback, props, start, first) {
        let elseRequests;
        if (props) {
    ...
    checkIfBlock(ifBlockCtx, tellCallback, start, ctx, first) {
            ...
            if (count) {
                ...
                if (elseIfArray) {
                    elseIfArray.forEach((elseIfCtx) => {
                        count = this.checkElse(elseIfCtx, tellCallback.bind(this), count, start, false);
                    });
                }
                // else
        ...
        // For others, check if the ifBlock has already been processed (if request in ifStatement/preceeding elseIfStatment)
            ...
            const ifRequests = this.checkIfStatement(ifBlockCtx.ifStatement().statementList(), tellCallback.bind(this), start, first);
            if (ifRequests) {
                if (elseIfArray) {
                ...
                const elseStatementCtx = ctx;
                count = this.checkElse(elseStatementCtx, tellCallback.bind(this), count, false, first);
            }
        }
        return count;
    }
    checkErrorHandler(parentCtx, tellCallback, start, first) {
        ...
        if (tryCount) {
            ...
            if (errorStatementListCtx) {
                const count = this.checkIfStatement(tryStatementListCtx, tellCallback.bind(this), start, first);
                if (count) {
                    tryCount[0] += count[0];
                    tryCount[1] += count[1];
                }
