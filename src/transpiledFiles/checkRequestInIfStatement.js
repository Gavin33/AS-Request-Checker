...
var checkRequestInIfStatement = /*#__PURE__*/function (_checkBlock_) {
  _inherits(checkRequestInIfStatement, _checkBlock_);
  ...
  function checkRequestInIfStatement(options, ifBlockCtx, ctx) {
    ...
    if ((ctx === null || ctx === void 0 ? void 0 : ctx.constructor.name) === 'IfStatementContext' || !ctx) {
      ...
      _this.checkStatement(ifBlockCtx.ifStatement().statementList());
      // elseIf
      if (elseIfArray) {
        elseIfArray.forEach(function (elseIfCtx) {
          _this.checkElse(elseIfCtx);
        });
      }
      // else
      var elseStatementCtx = ifBlockCtx.elseStatement();
      if (elseStatementCtx) {
        _this.checkElse(elseStatementCtx);
      }
    }
    // For others, check if the ifBlock has already been processed (if request in ifStatement/preceeding elseIfStatment)
      ...
      _this.checkStatement(ifBlockCtx.ifStatement().statementList());
      if (elseIfArray) {
        var _iterator = _createForOfIteratorHelper(elseIfArray),
          _step;
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            var elseIfCtx = _step.value;
            _this.checkElse(elseIfCtx);
          }
        } catch (err) {
          _iterator.e(err);
      ...
      _this.checkElse(_elseStatementCtx);
    }
    return _this;
  }
  _createClass(checkRequestInIfStatement, [{
    key: "checkElse",
    value: function checkElse(ctx) {
